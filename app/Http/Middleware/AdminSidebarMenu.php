            ...
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action('JournalEntriesController@index'),
                            __('lang_v1.add_Journal_entrie'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'Journal_entrie']
                        );

                        $sub->url(
                            action('CostCenterController@index'),
                            __('lang_v1.cost_centers'),
                            ['icon' => 'fa fas fa-cost-center', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'cost-center']
                        );
                        $sub->url(
